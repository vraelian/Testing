...
**MissionService (F018)**
Responsibility: Coordinates the Mission System 2.0 lifecycle. Delegates logic to evaluators. Handles freight depositing logic.
Dependencies: GameState, MissionTriggerEvaluator, MissionObjectiveEvaluator.

**MissionTriggerEvaluator**
Responsibility: Stateless logic delegate that evaluates "Can I see this mission?" based on prerequisite logic.
Dependencies: GameState.

**MissionObjectiveEvaluator**
Responsibility: Stateless logic delegate that evaluates "Is this objective met?" dynamically calculating fleet holds and deposited freight.
Dependencies: GameState.
...
Responsibility: Internal "Headless Player" bot for client-side economic simulation and validation.
Key Behavior: Utilizes Goal-Oriented Action Planning (GOAP) to bypass the input layer and execute trades/travel directly against the SimulationService to test long-term inflation or exploitation.
Dependencies: GameState, SimulationService, Logger.
...
* UIMissionControl: Manages Mission data screens, sticky bar HUD, Synthesized Objectives, and Intel interactions.
