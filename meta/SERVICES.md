...
Key Behavior: Validates actions against player credits/capacity, mutates GameState, logs transactions. Uses MOD_FUEL_PRICE for refueling costs. Now orchestrates fleet trading, storage mechanics, and dynamic fleet cost averaging.
...
UIMissionControl: Manages Mission data screens, the new `MissionsScreen` renderer, sticky bar HUD tracking, and Intel interactions.
...
UISolStationControl: Manages the Sol Station Dashboard, operational mode switching, cache grids, the in-modal Officer Roster sub-view, the Engineering Interface, and level-up animations.
...
ActionClickHandler (F039): Routes data-action clicks to services. Handles Upgrade Installation, ship cycling interactions on the services screen, and Travel initiation (including Consumable flags).
