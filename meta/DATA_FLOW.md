...
2.28 Act Screen Intermission Sequence
Flow for intercepting the standard simulation loop to display major narrative chapter transitions.

Code snippet
graph TD
    subgraph Trigger
        A[Mission / Story Event Completion] --> B[FlagMutator Identifies Act Threshold];
    end

    subgraph Interception
        B --> C[UIEventControl intercepts standard routing];
        C --> D[Pause standard Simulation / UIManager renders];
    end
    
    subgraph Cinematic Display
        D --> E[Web Animations API initializes overlay DOM];
        E --> F[Execute blocking fade-in and text reveal];
        F -- Await Interaction / Timer --> G[Execute fade-out cleanup];
    end
