...
* **Sol Station** | Exports: Antimatter | Imports: Graphene Lattices, Plasteel
* **Mercury** | Exports: Plasteel | Imports: Water Ice (Strong Import)
* **Venus** | Exports: Cloned Organs, Neural Processors | Imports: Sentient AI Cores, Atmo Processors
* **Earth** | Exports: Hydroponics, Cybernetics | Imports: Cloned Organs, Xeno-Geologicals
* **The Moon** | Exports: Plasteel, Graphene Lattices | Imports: Water Ice, Hydroponics
* **Mars** | Exports: Plasteel, Xeno-Geologicals | Imports: Hydroponics, Water Ice
* **The Belt** | Exports: Water Ice, Xeno-Geologicals | Imports: Hydroponics, Cybernetics
* **The Exchange** | Exports: Sentient AI Cores, Cloned Organs | Imports: Antimatter, Xeno-Geologicals
* **Jupiter** | Exports: Refined Propellant | Imports: Plasteel, Atmo Processors
* **Saturn** | Exports: Cybernetics | Imports: Cryo-Sleep Pods, Cloned Organs
* **Uranus** | Exports: Neural Processors | Imports: Sentient AI Cores, Atmo Processors
* **Neptune** | Exports: Cryo-Sleep Pods, Graphene Lattices | Imports: Plasteel, Refined Propellant
* **Kepler's Eye** | Exports: Antimatter | Imports: Folded-Space Drives, Neural Processors
* **Pluto** | Exports: Graphene Lattices, Xeno-Geologicals | Imports: Antimatter, Cybernetics
...
## V. Volume Constraints & Fleet Logistics
To prevent the "late-game singularity", the economy utilizes strict Volume over Value constraints coupled with Geometric Pricing and Fleet integration.
...
* **The Cargo Constraint (Sub-1000 Limit):** Ship cargo capacities are strictly hard-capped below 1,000 slots. Players must use lower-tier goods (Water Ice, Plasteel) as bulk "packing peanuts" to maximize the efficiency of their massive cargo bays alongside highly profitable top-tier goods. Legacy "Volumetric Sinks" (massive HP/Cargo numbers) have been permanently abolished to maintain UI readability and strict mechanical tension.
* **Fleet Cost Averaging:** When operating a multi-ship fleet, players can distribute massive cargo hauls across numerous vessels. To prevent inventory tracking exploits, the game calculates a single, dynamic `Average Cost Basis` for each commodity across the entire active fleet inventory. This mathematically prevents players from artificially manipulating profit margins by shuffling goods between ships.
...
* **Tier-Scaled Service Costs (Upkeep):** Fuel costs apply exponential class multipliers (up to 500x for Capital ships). Repair costs scale algorithmically off the ship's base price (`ShipPrice * 0.0001`). This acts as the primary endgame wealth tax.
* **Upgrade Economy (Hybrid Pricing):** Upgrades cost a Fixed Base + a percentage of the host ship's value, ensuring upgrades remain a significant investment even on low-tier chassis.
...
1.  **Bifurcated Debt Structures:**
    * **Guild Debt:** Highly regulated, offering lower interest (2%) but implementing strict 30-day garnishments that automatically deduct from player liquidity.
    * **Syndicate Debt:** High-risk, high-interest (7%) and lacks automatic garnishment, relying instead on punitive "Repo Strikes" if payments are ignored.
2.  **Solvency & The Repo Loop:** The `BankruptcyService` continually evaluates the player's debt-to-liquidity ratio. If a player holding Syndicate Debt enters insolvency (debt exceeds 150% of liquid assets plus fleet value), a Repo Strike is issued.
3.  **Asset Forfeiture:** Maxing out Repo Strikes forces a Syndicate Repo Event: the service identifies the highest value asset (a fleet ship or an installed Tier IV/V upgrade), forces a hostile liquidation, and seizes the credits to instantly clear the balance, destroying player progression.
