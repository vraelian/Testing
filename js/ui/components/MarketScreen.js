...
function _getMarketItemHtml(good, gameState, getItemPrice, marketTransactionState) {
    ...
    for (const shipId of player.ownedShipIds) {
        const item = player.inventories[shipId]?.[good.id];
    ...
    const fleetItem = totalQty > 0 ? { quantity: totalQty, avgCost: totalCostValue / totalQty } : null;
    ...
    if (hasLicense) {
        ...
        const indicatorHtml = renderIndicatorPills({ price: effectivePrice, sellPrice, galacticAvg, playerItem: fleetItem });
        const avgCostHtml = fleetItem ? `<div class="avg-cost-display" id="avg-cost-${good.id}">Avg. Cost: ${formatCredits(fleetItem.avgCost, false)}</div>` : '';
