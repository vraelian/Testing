...
import { ACTION_IDS, SHIP_IDS, GAME_RULES } 
    ...
    return `
        <div class="flex flex-col h-full">
            <div id="ship-terminal-container" class="flex flex-col flex-grow min-h-0 ${modeClass}">
                <div 
class="toggle-container mx-auto my-1">
 ...
 * @param 
{object} gameState The current game state.
...
function _renderShipCarouselPage(gameState, shipId, isHangarMode) {
    ...
    if (isHangarMode) {
        ...
        statusBadgeHtml = `<div class="status-badge" style="border-color: ${isActive ? 'var(--theme-color-primary)' : 'var(--ot-border-light)'}; color: ${isActive 
? 'var(--theme-color-primary)' : 'var(--ot-text-secondary)'};">${isActive ? 'ACTIVE' : 'STORED'}</div>`;
    }
...
function _renderSpecBar(label, current, max, color) {
    ...
    return `
        <div class="spec-readout hangar-specs">
            ...
            <div class="spec-bar"><div class="spec-bar-fill" style="width: ${percentage}%; background-color: ${color}; --bar-color: 
${color};"></div></div>
